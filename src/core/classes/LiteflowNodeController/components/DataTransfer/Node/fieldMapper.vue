<template>
  <div id="shape" @dblclick="eidtNode" :class="{ noData: noData }">
    <div class="fx">
      <div id="shapeIcon">
        <img src="/static/icons/FieldMapper2.svg" alt="" />
      </div>
      <div>{{ state.name }}</div>
    </div>
...
function changeNoData() {
  if (!state.field_mapper || isEmpty(state.field_mapper)) {
    noData.value = true
    return true
  }
  noData.value = false
...
function eidtNode() {
  ...
  dataTransferController.editFieldMapper(state, {}, nodes).then((res) => {
