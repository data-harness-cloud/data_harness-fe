...
import { DatabaseManagementController, RemoteHostController } from '@/api/index'
...
function initMonaco() {
  monaco.languages.registerCompletionItemProvider('shell', {
    provideCompletionItems: (model, position) => {
      ...
      if (lastContents) {
        const shellConfigKey = ['keywords', 'builtins']
        console.log('shellConfigKey', shellConfigKey)
        shellConfigKey.forEach((key) => {
          shellLanguage[key].forEach((shell) => {
            suggestions.push({
              label: shell, // 显示的提示内容;默认情况下，这也是选择完成时插入的文本。
              insertText: shell, // 选择此完成时应插入到文档中的字符串或片段
            })
          })
        })
      }
...
const remoteHosts = ref(null)
async function getDatabase() {
  await RemoteHostController.list(
    http,
    {
      remoteHostDtoFilter: {
    ...
    remoteHosts.value = res.data.dataList
  })
}
