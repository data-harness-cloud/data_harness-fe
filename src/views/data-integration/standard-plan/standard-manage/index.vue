<template>
  ...
  <div class="main">
    <TableTemplate
      v-if="standardMainController"
      :controller="standardMainController"
      :options="options"
      ref="tableTempRef"
    >
      <template #in-filter>
        ...
        <el-button
          ...
          @click="
            standardMainController
              ...
              .then(() => {
                standardMainController.refreshTable(true)
              })
          "
        >
          新增
      ...
      <template #slot-operate="{ scope }">
        <el-button
          ...
          size="large"
          @click="
            standardMainController.handleUpdate(scope?.row).then(() => {
              standardMainController.refreshTable(false)
            })
          "
        >
        ...
        <el-button
          ...
          size="large"
          @click="
            standardMainController.handleDelete(scope?.row).then(() => {
              standardMainController.refreshTable(true)
            })
          "
        >
...
import StandardMainController from '@/core/classes/StandardMainController/index'
const standardMainController = new StandardMainController()
...
function selectIndex(node) {
  ...
  standardMainController.dataFilter = {
    standardDirectoryId: node.id,
  }
  standardMainController.refreshTable(false, { showMask: false })
}
...
function publishStandardMain() {
  ...
  rows.forEach((element) => {
    if (element.standardStatus === '1') {
      standardMainController.$message.warning(element.standardName + '已发布')
      return
    }
    element.standardStatus = '1'
    standardMainController.update(element).then(() => {
      standardMainController.$message.success(element.standardName + '发布成功')
    })
  })
}
</script>
