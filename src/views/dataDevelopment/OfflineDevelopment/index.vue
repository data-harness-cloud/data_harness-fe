<template>
  <div style="position: relative; display: flex">
    ...
    <div class="x6main">
      <TaskEdit v-if="showEditTaskPlane" @back="undisplayEditTaskPlane" :defaultData="selectedTask" />
    </div>
  </div>
</template>
...
function displayEditTaskPlane(Task = null) {
  const ruler = liteflowRulerList.find((r) => r.id === Task.id)
  const planningProcess = DataPlanning.planningProcess.find((r) => r.id === Task.parentId)
  ruler.classificationId = planningProcess.classificationId
  ruler.processThemeId = planningProcess.processThemeId

  selectedTask.value = ruler
